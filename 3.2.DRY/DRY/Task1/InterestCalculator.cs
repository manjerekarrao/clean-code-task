...
namespace DRY.Task1
{
    ...
    {
        private const int _age = 60;
        private const double _interestPercent = 4.5d;
        private const double _seniorPercent = 5.5d;
        private const int _bonusAge = 13;
        ...
        {
            return DurationBetweenDatesInYears(accountDetails.Birth, accountDetails.StartDate) > _bonusAge;
        }
        ...
        private decimal Interest(AccountDetails accountDetails)
        {
            ...
            if (IsAccountStartedAfterBonusAge(accountDetails))
            {
                if (_age <= accountDetails.Age)
                {
                    //interest = (PrincipalAmount * DurationInYears * AnnualInterestRate) / 100
                    interest = (double) accountDetails.Balance
                               * DurationSinceStartDateInYears(accountDetails.StartDate) * _seniorPercent / 100;
                }
                else
                {
                    interest = (double) accountDetails.Balance
                               * DurationSinceStartDateInYears(accountDetails.StartDate) * _interestPercent / 100;
                }
            }
